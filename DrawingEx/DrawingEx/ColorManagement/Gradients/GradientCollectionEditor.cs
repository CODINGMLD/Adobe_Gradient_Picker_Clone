...
namespace DrawingEx.ColorManagement.Gradients
{
	/// <summary>
	...
	public partial class GradientCollectionEditor : Form
	{
		...
		private class GradientUICollection : GradientCollection
		{
			...
			{
				_owner.lstPresets.Elements.Insert(index, new GradientDisplayElement(value));
			}
			protected override void OnClearComplete()
			{
				_owner.lstPresets.Elements.Clear();
			}
			protected override void OnRemoveComplete(int index, Gradient value)
			{
				if (index < _owner.lstPresets.Elements.Count)
					_owner.lstPresets.Elements.RemoveAt(index);
			}
			protected override void OnSetComplete(int index, Gradient oldValue, Gradient newValue)
			{
				_owner.lstPresets.Elements[index] = new GradientDisplayElement(newValue);
			}
		}
		/// <summary>
		...
		private class GradientDisplayElement : DisplayItem
		{
			...
			protected override void OnDraw(PaintEventArgs e)
			{
						...
						Color.Transparent, Color.Transparent))
				{
					...
					e.Graphics.FillRectangle(lnbrs, e.ClipRectangle);
				}
			}
		...
		private void btnNewPreset_Click(object sender, EventArgs e)
		{
			...
			_gradients.Add(grd);
			//update ui
			lstPresets.Elements.SelectedIndex = lstPresets.Elements.Count - 1;
		}

		#endregion
		//
		private void lstPresets_SelectionChanged(DisplayList sender, EventArgs e)
		{
			GradientDisplayElement elem =
				lstPresets.Elements.SelectedElement as GradientDisplayElement;
			//
		...
		{
			if (lstPresets.Elements.SelectedElement != null)
				lstPresets.Elements.SelectedElement.RaiseRefresh();
		}

		private void mnuDelete_Click(object sender, EventArgs e)
		{
			if (lstPresets.Elements.SelectedElement != null)
				_gradients.Remove(((GradientDisplayElement)lstPresets.Elements.SelectedElement).Gradient);
		}
		...
		public Gradient SelectedGradient
		{
			...
			{
				edit.Gradient = value;
				lstPresets.Elements.SelectedIndex =
